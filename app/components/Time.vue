...
const formattedDate = computed(() => {
  ...
  const units: Array<{
    ...
    { unit: 'day', seconds: 86_400, threshold: 30 }, // ~30 days → month
    { unit: 'month', seconds: 2_592_000, threshold: 12 }, // 12 months → year
    { unit: 'year', seconds: 31_536_000, threshold: Infinity },
  ]
...
const title = computed(() =>
  props.title === true ? isoDate.value : typeof props.title === 'string' ? props.title : undefined,
)
  ...
  onPrehydrate((el) => {
    ...
    for (const name of el.getAttributeNames()) {
      if (name.startsWith('data-')) {
        ...
        options[optionName] = (
          attrValue === 'true' ? true : attrValue === 'false' ? false : attrValue
        ) as any
      }
    }
...
<template>
  <time v-bind="dataset" :datetime="isoDate" :title="title">{{ formattedDate }}</time>
</template>
